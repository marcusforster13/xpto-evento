...
function emailConvidado(nome, adultos, criancas, intolerances, obs) {
  const pessoas = Number(adultos) + Number(criancas);
...
function emailTime(nome, email, tipo, adultos, criancas, campeonato, jogo, jogoModo, intolerances, obs) {
  const jogoLabel = { cs: 'Counter-Strike', fifa: 'FIFA' }[jogo] || '—';
  const modoLabel = { duo: 'Duo (2 jogadores)', squad: 'Squad (4 jogadores)', solo: 'Individual' }[jogoModo] || '';
  const campLabel = campeonato === 'sim' ? `Sim — ${jogoLabel}${modoLabel ? ` (${modoLabel})` : ''}` : 'Não, só vai assistir';
  const pessoas = Number(adultos) + Number(criancas);
  const pessoasLabel = criancas > 0
    ? `${pessoas} (${adultos} adulto${adultos != 1 ? 's' : ''} + ${criancas} criança${criancas != 1 ? 's' : ''})`
          ...
          <td style="padding:28px 32px;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #eee;border-radius:6px;overflow:hidden;font-size:14px;">
              ...
              <tr>
                ...
                <td style="padding:10px 16px;color:#111;border-top:1px solid #eee;">${pessoasLabel}</td>
              </tr>
              <tr style="background:#fafafa;">
              ...
              <tr>
                ...
                <td style="padding:10px 16px;color:#111;border-top:1px solid #eee;">${campLabel}</td>
              </tr>
              ${obs ? `<tr>
...
export default async function handler(req) {
  ...
  const { nome, email, tipo, adultos = 1, criancas = 0, campeonato = '', jogo = '', jogo_modo = '', 'intolerâncias': intolerances = [], observacoes = '' } = body;
  ...
  try {
    await Promise.all([
      ...
      sendEmail({
        ...
        html: emailConvidado(nome, adultos, criancas, intolerances, observacoes),
      }),
      // 2. Notificação para o time
      sendEmail({
        to: EMAIL_TEAM,
        subject: `[Evento 13.06] Nova confirmação: ${nome}`,
        html: emailTime(nome, email, tipo, adultos, criancas, campeonato, jogo, jogo_modo, intolerances, observacoes),
      }),
    ]);
